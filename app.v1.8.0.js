...
async function refreshRemoteSegment(segment = 'all', options = {}) {
  ...
  try {
    ...
    if (segment === 'category_memory' || segment === 'all') {
      state.categoryMemory = await store.getCategoryMemory();
    }
...
function handleRealtimeChange(tableName, payload) {
  ...
  if (tableName === 'items') scheduleRefresh('items', 120);
  else if (tableName === 'rules') scheduleRefresh('rules', 150, { silent: true });
  else if (tableName === 'stores') scheduleRefresh('stores', 150);
  else if (tableName === 'categories') scheduleRefresh('categories', 150);
  else if (tableName === 'category_memory') scheduleRefresh('category_memory', 150, { silent: true });
  else if (tableName === 'note_items') scheduleRefresh('notes', 120);
}
...
function bindConnectivityRefresh() {
  ...
  function getCategoryMemoryEntry(normalizedName) {
    return state.categoryMemory.find((entry) => entry.item_key === normalizedName);
  }
  ...
  async function saveCategoryMemory(itemKey, category) {
    ...
    beginWrite();
    try {
      await store.saveCategoryMemory({
        household_id: SHARED_SCOPE_ID,
        item_key: itemKey,
        category,
        updated_at: new Date().toISOString(),
