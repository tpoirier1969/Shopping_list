(function () {
  ...
  class SupabaseStore {
    ...
    async getStores() {
      const { data, error } = await this.client
        ...
        .order('created_at', { ascending: true });
      if (error) {
        if (error.code === '42P01') return [...BUILTIN_STORES];
        throw error;
      }
      const stores = data || [];
      ...
      <section class="view active" id="view-startup-error">
        <div class="panel">
          ...
          <div class="empty-state" style="text-align:left;line-height:1.5;">
            ...
            <div style="margin-top:12px;">Check your Supabase values, make sure Anonymous sign-ins are enabled in Supabase Auth, then hard refresh the page.</div>
          </div>
        </div>
      </section>
    `;
  ...
  function openStoreModal() {
    ...
    setTimeout(() => els.storeNameInput.focus(), 50);
  }
  ...
  async function loadAll() {
    ensureStoreReady();
    const [items, rules, note, stores] = await Promise.all([
      store.getItems(),
      store.getRules(),
      store.getNote(),
      store.getStores(),
    ...
    state.rules = rules;
    state.note = note;
    state.stores = mergeStores(stores);
    renderAll();
  }
  ...
  function hasTabFlag(tabName) {
    ...
    if (tabName === 'parents') {
      return getItemsForTab('parents').length > 0;
    }
    if (tabName === 'notes') {
      return Boolean((state.note || '').trim());
    }
    return false;
  }
    ...
    const rows = items.map((item) => `
      <div class="item-row removed-row">
        ...
        <div class="item-meta">
          ...
          ${item.parent_target ? `<span class="parent-badge">${escapeHtml(PARENT_LABELS[item.parent_target] || PARENT_LABELS.schaffer)}</span>` : ''}
        </div>
    ...
    return `
      <div class="${rowClass}">
        ...
        <div class="item-main">
          <span class="item-name">${escapeHtml(item.item_name)}</span>
          <div class="item-meta">
            ${showParentBadge ? `<span class="parent-badge">${escapeHtml(PARENT_LABELS[item.parent_target] || PARENT_LABELS.schaffer)}</span>` : ''}
    ...
    document.querySelectorAll('[data-action]').forEach((button) => {
      button.addEventListener('click', async (event) => {
        ...
        try {
          ...
          if (action === 'open-store-modal') openStoreModal();
          if (action === 'remove') await removeItem(id);
          if (action === 'restore') await restoreItem(id);
          if (action === 'done-store') await shoppedStore(storeName);
          if (action === 'delivered-parent') await deliveredParent(parentKey);
        } catch (error) {
          handleError(error);
        }
      });
