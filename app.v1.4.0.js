(function () {
  ...
  const CACHE_KEY = 'shared-shopping-list-cache-v1';
  ...
  function renderTabBarAndViews() {
    ...
    els.tabBar.innerHTML = [
      ...
      `),
      `<button class="tab tab-add-store" data-action="open-store-modal" ${state.ready ? '' : 'disabled'}>＋ Store</button>`,
    ].join('');
  ...
  function renderStoreView(storeKey) {
    ...
    const actions = `
      <button class="control-btn" data-action="open-add" data-store="${escapeHtml(storeKey)}" ${state.ready ? '' : 'disabled'}>Add item</button>
      <button class="control-btn primary" data-action="done-store" data-store="${escapeHtml(storeKey)}" ${(state.ready && items.some((item) => item.purchased_main)) ? '' : 'disabled'}>Done</button>
    `;
