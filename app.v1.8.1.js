(function () {
  ...
  const SUPABASE_SCHEMA = 'tod_donna_shared_shopping';
  ...
  const CACHE_KEY = 'shared-shopping-list-cache-v5';
  ...
  const state = {
    ...
    builderSortables: [],
    categoryMemory: [],
    categoryPickerSnapshot: '',
    cache: {
      categoryOrder: [...CATEGORY_ORDER],
      categoryOrderSet: new Set(CATEGORY_ORDER.map((entry) => entry.toLowerCase())),
      storeDefs: [...BUILTIN_STORES],
      storeDefsByKey: new Map(),
      storeFilterDefs: [{ key: 'master', label: 'Master List', categories: [...CATEGORY_ORDER] }],
      storeFilterByKey: new Map(),
      categoryMemoryByKey: new Map(),
      ruleByCompositeKey: new Map(),
    },
    itemModal: {
  ...
  const els = {
    ...
    categoryRememberWrap: document.getElementById('categoryRememberWrap'),
    categoryRememberCheckbox: document.getElementById('categoryRememberCheckbox'),
  ...
  function rebuildStateCaches() {
    const categoryOrder = mergeCategories(state.categories);
    const storeDefs = mergeStores(state.stores);
    const storeDefsByKey = new Map(storeDefs.map((entry) => [entry.store_key, entry]));
    const storeFilterDefs = [
      { key: 'master', label: 'Master List', categories: [...categoryOrder] },
      ...storeDefs
    ...
    const storeFilterByKey = new Map(storeFilterDefs.map((entry) => [entry.key, entry]));
    const categoryMemoryByKey = new Map();
    (state.categoryMemory || []).forEach((entry) => {
      if (entry?.item_key) categoryMemoryByKey.set(entry.item_key, entry);
    });
    const ruleByCompositeKey = new Map();
    (state.rules || []).forEach((rule) => {
      if (!rule?.item_key) return;
      ruleByCompositeKey.set(`${rule.store || ''}::${rule.item_key}`, rule);
    });
    state.cache = {
      categoryOrder,
      categoryOrderSet: new Set(categoryOrder.map((entry) => entry.toLowerCase())),
      storeDefs,
      storeDefsByKey,
      storeFilterDefs,
      storeFilterByKey,
  ...
  function refreshCategoryPickerOptions(force = false) {
    const categories = getCategoryOrder();
    const snapshot = categories.join('||');
    if (!force && snapshot === state.categoryPickerSnapshot) return;
    els.categoryPicker.innerHTML = categories
      .map((category) => `<option value="${escapeHtml(category)}">${escapeHtml(category)}</option>`)
      .join('');
    state.categoryPickerSnapshot = snapshot;
  }
  ...
  class SupabaseStore {
    ...
    table(tableName) {
      return this.client.schema(SUPABASE_SCHEMA).from(tableName);
    }

    async getItems() {
      const { data, error } = await this.table('items')
        .select('*')
    ...
    async getRules() {
      const { data, error } = await this.table('rules')
        .select('*')
        .eq('household_id', this.householdId);
      if (error) throw error;
      return data || [];
    ...
    async getLegacyNote() {
      const { data, error } = await this.table('notes')
        .select('*')
        .eq('household_id', this.householdId)
        .limit(1);
      if (error) throw error;
    ...
    async saveLegacyNote(body) {
      ...
      const { error } = await this.table('notes')
    ...
    async getNoteItems() {
      const { data, error } = await this.table('note_items')
        .select('*')
        .eq('household_id', this.householdId)
    ...
    async deleteNoteItem(id) {
      const { error } = await this.table('note_items')
        .delete()
        .eq('household_id', this.householdId)
        .eq('id', id);
      if (error) throw error;
    ...
    async getCategories() {
      const { data, error } = await this.table('categories')
        .select('*')
        .eq('household_id', this.householdId)
        .order('sort_order', { ascending: true })
    ...
    async getCategoryMemory() {
      const { data, error } = await this.table('category_memory')
        .select('*')
        .eq('household_id', this.householdId)
    ...
    async saveCategoryMemory(memoryDef) {
      const payload = { ...memoryDef, household_id: this.householdId };
      const { data, error } = await this.table('category_memory')
        .upsert(payload, { onConflict: 'household_id,item_key' })
        .select()
        .single();
      if (error) throw error;
      return data;
    ...
    async getStores() {
      const { data, error } = await this.table('stores')
        .select('*')
        .eq('household_id', this.householdId)
        .order('sort_order', { ascending: true })
        .order('created_at', { ascending: true });
...
startRealtime(onChange, onStatus) {
  ...
  ['items', 'rules', 'stores', 'categories', 'category_memory', 'note_items'].forEach((tableName) => {
    channel.on('postgres_changes', { event: '*', schema: SUPABASE_SCHEMA, table: tableName }, (payload) => {
      try {
...
stopRealtime() {
  ...
  function readCachedSnapshot() {
    try {
      ...
      return {
        ...
        categoryMemory: Array.isArray(parsed.categoryMemory) ? parsed.categoryMemory : [],
  ...
  function getCategoryOrder() {
    return state.cache.categoryOrder || [...CATEGORY_ORDER];
  }

  function getStoreDefs() {
    return state.cache.storeDefs || [...BUILTIN_STORES];
  }
  ...
  function getStoreFilterDefs() {
    return state.cache.storeFilterDefs || [{ key: 'master', label: 'Master List', categories: getCategoryOrder() }];
  }

  function getStoreFilterDef(filterKey) {
    return state.cache.storeFilterByKey?.get(filterKey) || getStoreFilterDefs()[0];
  }
  ...
  function getStoreLabel(storeKey) {
    return state.cache.storeDefsByKey?.get(storeKey)?.store_label || storeKey;
  }
...
function upsertByKey(list, row, keyFn) {
  const key = keyFn(row);
  const next = Array.isArray(list) ? [...list] : [];
  const index = next.findIndex((entry) => keyFn(entry) === key);
  if (index >= 0) next[index] = { ...next[index], ...row };
  else next.push(row);
  return next;
}

function removeByKey(list, row, keyFn) {
  const key = keyFn(row);
  return (Array.isArray(list) ? list : []).filter((entry) => keyFn(entry) !== key);
}

function upsertStateItem(saved) { state.items = upsertByKey(state.items, saved, (entry) => entry.id); }
function upsertStateNote(saved) { state.noteItems = upsertByKey(state.noteItems, saved, (entry) => entry.id); }
function upsertStateRule(saved) {
  state.rules = upsertByKey(state.rules, saved, (entry) => `${entry.store || ''}::${entry.item_key}`);
  rebuildStateCaches();
}
function upsertStateCategoryMemory(saved) {
  state.categoryMemory = upsertByKey(state.categoryMemory, saved, (entry) => entry.item_key);
  rebuildStateCaches();
}
function upsertStateStore(saved) {
  state.stores = upsertByKey(state.stores, saved, (entry) => entry.store_key);
  rebuildStateCaches();
}
function upsertStateCategory(saved) {
  const categoryName = normalizeCategoryName(saved?.category_name || saved);
  if (!categoryName) return;
  const next = (state.categories || []).filter((entry) => normalizeCategoryName(entry).toLowerCase() !== categoryName.toLowerCase());
  next.push(categoryName);
  state.categories = mergeCategories(next);
  rebuildStateCaches();
}
function applyRealtimeRow(tableName, payload) {
  const eventType = payload?.eventType || 'UPDATE';
  const row = eventType === 'DELETE' ? payload?.old : (payload?.new || payload?.old);
  if (!row) return false;
  if (row.household_id && row.household_id !== SHARED_SCOPE_ID) return false;
  if (tableName === 'items') { state.items = eventType === 'DELETE' ? removeByKey(state.items, row, (entry) => entry.id) : upsertByKey(state.items, row, (entry) => entry.id); return true; }
  if (tableName === 'rules') {
    state.rules = eventType === 'DELETE' ? removeByKey(state.rules, row, (entry) => `${entry.store || ''}::${entry.item_key}`) : upsertByKey(state.rules, row, (entry) => `${entry.store || ''}::${entry.item_key}`);
    rebuildStateCaches();
    return true;
  }
  if (tableName === 'stores') {
    state.stores = eventType === 'DELETE' ? removeByKey(state.stores, row, (entry) => entry.store_key) : upsertByKey(state.stores, row, (entry) => entry.store_key);
    rebuildStateCaches();
    return true;
  }
  if (tableName === 'categories') {
    const categoryName = normalizeCategoryName(row.category_name);
    if (!categoryName) return false;
    state.categories = eventType === 'DELETE' ? (state.categories || []).filter((entry) => normalizeCategoryName(entry).toLowerCase() !== categoryName.toLowerCase()) : mergeCategories([...(state.categories || []), categoryName]);
    rebuildStateCaches();
    return true;
  }
  if (tableName === 'category_memory') {
    state.categoryMemory = eventType === 'DELETE' ? removeByKey(state.categoryMemory, row, (entry) => entry.item_key) : upsertByKey(state.categoryMemory, row, (entry) => entry.item_key);
    rebuildStateCaches();
    return true;
  }
  if (tableName === 'note_items') { state.noteItems = eventType === 'DELETE' ? removeByKey(state.noteItems, row, (entry) => entry.id) : upsertByKey(state.noteItems, row, (entry) => entry.id); return true; }
  return false;
}
...
async function refreshRemoteSegment(segment = 'all', options = {}) {
  ...
  try {
    ...
    if (segment === 'rules' || segment === 'all') { state.rules = await store.getRules(); }
    if (segment === 'stores' || segment === 'all') { state.stores = await store.getStores(); }
    if (segment === 'categories' || segment === 'all') { state.categories = mergeCategories(await store.getCategories()); }
    if (segment === 'category_memory' || segment === 'all') { state.categoryMemory = await store.getCategoryMemory(); }
    if (segment === 'notes' || segment === 'all') { state.noteItems = await store.getNoteItems(); }
    rebuildStateCaches();
    writeCachedSnapshot();
    renderAll(segment === 'stores' || segment === 'categories' || segment === 'all');
    if (state.pendingWrites === 0) {
...
function handleRealtimeChange(tableName, payload) {
  const applied = applyRealtimeRow(tableName, payload);
  if (!applied) return;
  writeCachedSnapshot();
  renderAll(tableName === 'stores' || tableName === 'categories');
  if (state.pendingWrites === 0) setSyncBadge(state.liveChannelReady ? 'Live' : 'Saved', state.liveChannelReady ? 'live' : 'saved');
}
...
function bindConnectivityRefresh() {
  ...
  function mapToKnownCategory(categoryName) {
    const raw = normalizeCategoryName(categoryName);
  ...
  function getCategoryMemoryEntry(normalizedName) {
    return state.cache.categoryMemoryByKey?.get(normalizedName) || null;
  }

  function getLearnedRule(normalizedName, storeName = 'shopping') {
    return state.cache.ruleByCompositeKey?.get(`${storeName}::${normalizedName}`)
      || state.cache.ruleByCompositeKey?.get(`shopping::${normalizedName}`)
      || state.cache.ruleByCompositeKey?.get(`master::${normalizedName}`)
      || state.cache.ruleByCompositeKey?.get(`::${normalizedName}`)
      || null;
  }
  ...
  function guessCategory(itemName, storeName = 'shopping') {
    ...
    const remembered = getCategoryMemoryEntry(normalized);
    if (remembered?.category) return mapToKnownCategory(remembered.category);

    const learned = getLearnedRule(normalized, storeName);
    if (learned?.category) return mapToKnownCategory(learned.category);
    ...
    for (const rule of BUILTIN_RULES) {
      for (const keyword of rule.keywords) {
        ...
        if (score > bestScore) {
          bestScore = score;
          bestCategory = mapToKnownCategory(rule.category);
        }
      }
    }
    return bestCategory;
  ...
  function updateCategoryControls() {
    const selected = normalizeCategoryName(els.categoryPicker.value || '');
    const suggested = normalizeCategoryName(state.itemModal.suggestedCategory || 'Other') || 'Other';
    const hasName = Boolean(normalizeName(els.itemNameInput.value || ''));
    const isChanged = hasName && selected && selected !== suggested;
    els.categoryRememberWrap.classList.toggle('hidden', !isChanged);
    if (!isChanged) {
      els.categoryRememberCheckbox.checked = true;
      state.itemModal.rememberChoice = true;
    }
  }

  function syncSuggestedCategoryFromInput() {
    const suggested = guessCategory(els.itemNameInput.value || '', state.modalStore) || 'Other';
    state.itemModal.suggestedCategory = suggested;
    refreshCategoryPickerOptions();
    els.categoryPicker.value = suggested;
    state.itemModal.rememberChoice = true;
    els.categoryRememberCheckbox.checked = true;
  ...
  function getEffectiveCategory(item) {
    const raw = mapToKnownCategory(item?.category);
    if (getCategoryOrder().includes(raw)) return raw;
  ...
  function openModal(storeName) {
    ...
    els.itemForm.reset();
    state.itemModal.suggestedCategory = 'Other';
    state.itemModal.rememberChoice = true;
    els.categoryPicker.value = 'Other';
    els.categoryRememberCheckbox.checked = true;
    els.categoryRememberWrap.classList.add('hidden');
  ...
  async function saveRule(itemKey, category, storeName) {
    ...
    beginWrite();
    try {
      const saved = await store.saveRule({ household_id: SHARED_SCOPE_ID, item_key: itemKey, category, store: storeName, updated_at: new Date().toISOString() });
      upsertStateRule(saved);
      writeCachedSnapshot();
      finishWrite(true);
    } catch (error) {
      finishWrite(false);
  ...
  async function saveCategoryMemory(itemKey, category) {
    ensureStoreReady();
    beginWrite();
    try {
      const saved = await store.saveCategoryMemory({ household_id: SHARED_SCOPE_ID, item_key: itemKey, category, updated_at: new Date().toISOString() });
      upsertStateCategoryMemory(saved);
      writeCachedSnapshot();
      finishWrite(true);
    } catch (error) {
      finishWrite(false);
  ...
  async function addStoreFromModal() {
    ...
    beginWrite();
    try {
      const saved = await store.saveStore({ household_id: SHARED_SCOPE_ID, store_key: key, store_label: label, sort_order: maxOrder + 10 });
      upsertStateStore(saved);
  ...
  async function addCategoryFromBuilder() {
    ...
    beginWrite();
    try {
      const saved = await store.saveCategory({ household_id: SHARED_SCOPE_ID, category_name: categoryName, sort_order: maxSort + 10, is_builtin: false });
      upsertStateCategory(saved);
      if (input) input.value = '';
  ...
  async function addItemFromModal() {
    ...
    const normalized = normalizeName(itemName);
    const category = mapToKnownCategory(els.categoryPicker.value || state.itemModal.suggestedCategory || 'Other');
    const item = baseItem(itemName, category, 'shopping', currentParentSelection());

    await persistItem(item);

    if (!state.itemModal.suggestedCategory || category !== state.itemModal.suggestedCategory) {
      if (state.itemModal.rememberChoice) {
        await saveCategoryMemory(normalized, category);
      }
    }
  ...
  function renderTabBarAndViews() {
    ...
    `).join('');
    const existingKeys = Array.from(els.mainGrid.querySelectorAll('.view')).map((view) => view.id.replace('view-', ''));
    const nextKeys = tabs.map((tab) => tab.key);
    const needsRebuild = existingKeys.length !== nextKeys.length || existingKeys.some((key, index) => key !== nextKeys[index]);
    if (needsRebuild) {
      els.mainGrid.innerHTML = tabs.map((tab) => `
        <section id="view-${escapeHtml(tab.key)}" class="view ${tab.key === state.currentTab ? 'active' : ''}"></section>
      `).join('');
    } else {
      nextKeys.forEach((tabKey) => { const view = getView(tabKey); if (view) view.classList.toggle('active', tabKey === state.currentTab); });
    }
  }
  ...
  function renderStoreBuilder() {
    ...
    if (!view) return;
    const editableStores = getEditableStoreDefs();
    const isEditingExisting = state.storeBuilder.selectedStoreKey !== BUILDER_NEW_STORE_KEY;
    const selectedStore = editableStores.find((entry) => entry.store_key === state.storeBuilder.selectedStoreKey) || null;
    ...
    const builderModeText = isEditingExisting
      ? `Editing ${escapeHtml(selectedStore?.store_label || state.storeBuilder.workingLabel || 'store')}`
      : 'Creating a new store';
    const saveButtonText = isEditingExisting ? 'Update store' : 'Create store';
    const storeQuickButtons = editableStores.length ? `
      <div class="store-filter-bar">
        ${editableStores.map((storeDef) => `
          <button
            class="store-filter-btn ${storeDef.store_key === state.storeBuilder.selectedStoreKey ? 'active' : ''}"
            type="button"
            data-action="builder-select-store"
            data-store-key="${escapeHtml(storeDef.store_key)}"
          >
            ${escapeHtml(storeDef.store_label)}
          </button>
        `).join('')}
      </div>
    ` : '';
        ...
        <section class="builder-card">
          <div class="builder-card-head">
            <div>
              ...
              <div class="builder-card-sub">${builderModeText}</div>
          ...
          <div class="builder-actions-row">
            <button class="primary-btn" type="button" data-action="builder-save-store" ${state.ready ? '' : 'disabled'}>${saveButtonText}</button>
            <button class="secondary-btn" type="button" data-action="builder-reset-store" ${state.ready ? '' : 'disabled'}>Revert</button>
  ...
  function renderViewForTab(tabName) {
    if (tabName === 'shopping') renderShoppingView();
    else if (tabName === 'parents') renderParents();
    else if (tabName === 'notes') renderNotes();
    else if (tabName === 'trips') renderTrips();
    else if (tabName === 'stores') renderStoreBuilder();
    else if (tabName === 'removed') { const removedView = getView('removed'); if (removedView) removedView.innerHTML = renderRemoved(getItemsForTab('removed')); }
  }

  function renderAll(refreshPicker = false) {
    ensureCurrentStoreFilter();
    renderTabBarAndViews();
    renderViewForTab(state.currentTab);
    if (refreshPicker) refreshCategoryPickerOptions();
    bindDynamicEvents();
    switchTab(state.currentTab, { skipRender: true });
    if (state.currentTab === 'stores') initStoreBuilderSortables();
    else destroyBuilderSortables();
  }
  ...
  async function persistItems(items) {
    ...
    beginWrite();
    try {
      const savedRows = await Promise.all(items.map((item) => store.saveItem(item)));
      savedRows.forEach((saved) => upsertStateItem(saved));
      writeCachedSnapshot();
      renderAll();
      finishWrite(true);
      return savedRows;
  ...
  async function shoppedStore(storeName) {
    ...
    items.forEach((item) => {
      item.purchased_main = false;
      if (item.parent_target && storeName === 'shopping') item.on_shopping_list = false;
      else {
        item.removed = true;
  ...
  function switchTab(tabName, options = {}) {
    ...
    if (tabName === 'stores') initStoreBuilderSortables(); else destroyBuilderSortables();
    ...
    document.querySelectorAll('[data-action]').forEach((button) => {
      button.addEventListener('click', async (event) => {
        ...
        try {
          ...
          else if (action === 'builder-select-store') {
            selectBuilderStore(event.currentTarget.dataset.storeKey || BUILDER_NEW_STORE_KEY);
            renderAll();
          }
          else if (action === 'builder-reset-store') {
            selectBuilderStore(state.storeBuilder.selectedStoreKey || BUILDER_NEW_STORE_KEY);
            renderAll();
          }
  ...
  async function loadAll() {
    ...
    const [rules, legacyNote, stores, noteItems, categories, categoryMemory] = await Promise.all([
      store.getRules(),
      store.getLegacyNote(),
      store.getStores(),
      store.getNoteItems(),
      store.getCategories(),
      store.getCategoryMemory(),
    ]);
    ...
    state.categoryMemory = categoryMemory || [];
    rebuildStateCaches();
  ...
  async function initStore() {
    ...
    try {
      ...
      if (cached?.items?.length) {
        ...
        state.categoryMemory = cached.categoryMemory || [];
      ...
      const message = error.code === '42P01'
        ? 'Supabase setup issue: run the latest SQL and expose the custom schema tod_donna_shared_shopping in Supabase API settings.'
  ...
  function bindStaticEvents() {
    ...
    els.itemNameInput.addEventListener('input', () => {
      syncSuggestedCategoryFromInput();
    });
    ...
    els.categoryRememberCheckbox.addEventListener('change', () => {
      state.itemModal.rememberChoice = els.categoryRememberCheckbox.checked;
    });
